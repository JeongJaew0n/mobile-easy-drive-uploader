...
- 아래로 끌어 닫기 `Modifier.swipeToDismiss`(확대 중 비활성). 진입/복귀 전환은 `AppNavigation` 의 `MediaViewerKey` 메타데이터(fade + scale 0.92). 그리드 썸네일과 `thumb-<id>` 메모리 캐시 키를 공유해 원본 로드 전까지 썸네일을 보여준다.
