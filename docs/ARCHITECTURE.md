...
- **출처 탭**(전체·카메라·스크린샷·다른 앱)은 `MediaItem.relativePath` 로만 가른다(`feature/gallery/MediaSource.kt`). 메모리 필터 중 **가장 먼저** 적용해 `dayCounts`(기간 달력)도 그 탭 범위로 좁힌다. 다만 앨범(이동 대상) 목록은 탭 적용 전 목록에서 뽑는다 — 설계는 `GALLERY_SOURCE_TABS.md`.
...
- 아래로 끌어 닫기 `Modifier.swipeToDismiss`(확대 중 비활성). 진입/복귀 전환은 `AppNavigation` 의 전역 `transitionSpec` 이 목적지 키를 보고 분기(상세보기 fade + scale 0.92; 히어로가 있으면 갤러리를 `ExitTransition.KeepUntilTransitionsFinished` 로 불투명하게 남기고 뷰어만 fadeIn — 두 화면을 동시에 반투명하게 하면 윈도우 배경이 비쳐 번쩍인다. 뷰어에서 뒤로는 갤러리 `EnterTransition.None` + 뷰어 축소·페이드). 다크 모드 윈도우 배경은 `values-night/themes.xml` 에서 검정. 그리드 썸네일과 `thumb-<id>` 메모리 캐시 키를 공유해 원본 로드 전까지 썸네일을 보여준다.
