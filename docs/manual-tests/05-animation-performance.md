...
| ANI-20 | 갤러리에서 썸네일 탭 | 썸네일이 제자리에서 커지며 상세보기로 이어짐(히어로), 끝난 뒤 이미지가 튀지 않음. 회전 후 같은 항목 재진입 시엔 히어로 없이 열림(정상) | ✅ 2026-09-08 | 처음엔 ❌ '찰칵' 흰색 번쩍: 뷰어 fadeIn + 갤러리 fadeOut 이 겹치는 순간 흰 윈도우 배경이 비침 → 갤러리 `KeepUntilTransitionsFinished` + 다크 윈도우 배경(`VIEWER_STABILITY.md` §7). 5x 배율 중간 프레임에서 확장·어두워짐 확인 |
| ANI-21 | 히어로 진행 중 화면 | 배경이 어둡게 전환되고 상·하단 바는 바로 표시 | ✅ 2026-09-08 | 갤러리가 어두워지는 위로 썸네일이 커지고 상·하단 바가 슬라이드 인 |
| ANI-22 | 콜드 스타트 후 첫 그리드 / 빠른 스크롤 | 썸네일이 페이드 없이 즉시 표시(전역 crossfade 제거), 첫 목록도 페이드 없이 표시 | ✅ 2026-09-08 | 콜드 스타트 연속 캡처: 스피너 → 전체 그리드가 한 번에 불투명 표시, 조각 페이드 없음 |
