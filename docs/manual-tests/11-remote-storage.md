...
| RS-05 | S3 탐색에서 새 폴더·이름 변경·이동·삭제(파일·폴더) | 폴더 이름 변경/이동은 아래 오브젝트를 전부 복사 후 삭제(개수에 비례해 느림) — 하단 진행바에 "n / total 개 처리 중". 삭제는 확인 다이얼로그 후 즉시(휴지통 없음) | ⬜ | |
...
| RS-13 | 설정 계정 행 ⋮ → 수정 → 표시 이름만 바꿔 저장 | 폼에 기존 값 채워짐(종류 칩 없음, 비밀 비어 있음), 저장 후 목록 이름 갱신·연결 그대로 동작. 비밀을 새로 넣으면 그 값으로 연결 | ⬜ | `update` + `evict` |
