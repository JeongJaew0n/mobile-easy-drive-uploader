...
| RS-05 | S3 탐색에서 새 폴더·파일 이름 변경·파일 이동·삭제 | 폴더 이름 변경/이동 메뉴는 없음(FOLDER_MUTATION 없음). 삭제는 확인 다이얼로그 후 즉시(휴지통 없음) | ⬜ | |
...
| NAS-06 | 자체 서명 인증서 NAS | 현재 연결 실패(TLS). 지문 고정 옵션은 후속(`NAS_STORAGE.md` §2) | ⬜ | |
