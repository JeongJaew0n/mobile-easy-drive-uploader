...
- 2026-09-09 3차: S3 폴더 이름 변경·이동은 **대상 접두어가 이미 있으면 거부**한다(전에는 조용히 합쳐지고 같은 이름 오브젝트를 덮어썼다 — WebDAV 의 `Overwrite: F` 와 동작을 맞춤). 재개 업로드는 원격 파일이 사라졌으면 새로 만들지 않는다(만들면 앞 offset 바이트가 0 으로 남은 파일이 완료로 기록된다). 일괄 작업 진행률이 100% 까지 올라간다.
- 알려진 한계: 폴더 이동 중간에 실패하면 옛 접두어와 새 접두어에 절반씩 남는다(롤백 없음). 영상 압축 캐시가 비워져 재인코딩되면 세션이 소스 바이트에 묶여 있지 않아 이어 붙인 결과가 깨질 수 있다 — 세션 지문 저장이 필요하고 아직 하지 않았다.
