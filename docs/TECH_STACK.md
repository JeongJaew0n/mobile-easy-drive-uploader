...
| 다중 클라우드·NAS | 자체 `RemoteStorage` 추상화 + **S3 SigV4 직접 구현**(AWS SDK 미사용), WebDAV 는 OkHttp 로 직접, SMB2/3 는 **smbj** 0.15.0, SFTP 는 **sshj** 0.40.0(둘이 BouncyCastle·asn-one 을 공유하고 bcpkix 는 제외), 비밀은 Android Keystore AES-GCM(`security-crypto` 미사용) | Naver Cloud·KT Cloud·AWS·MinIO·NAS(WebDAV/SMB/SFTP)·Windows 공유 지원 — `MULTI_CLOUD.md`, `NAS_STORAGE.md` |
