...
| 다중 클라우드·NAS | 자체 `RemoteStorage` 추상화 + **S3 SigV4 직접 구현**(AWS SDK 미사용), WebDAV 는 OkHttp 로 직접, SMB2/3 는 **smbj** 0.15.0(유일한 추가 의존성, BouncyCastle 포함), 비밀은 Android Keystore AES-GCM(`security-crypto` 미사용) | Naver Cloud·KT Cloud·AWS·MinIO·NAS(WebDAV/SMB)·Windows 공유 지원 — `MULTI_CLOUD.md`, `NAS_STORAGE.md` |
