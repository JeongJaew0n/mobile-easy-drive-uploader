...
자동 백업(`AutoBackupUseCase`)이 이미 "새로 추가된 항목"을 주기적으로 훑는다. 같은 트리거에 얹지 않고 **별도 주기 워커**를 둔다 — 자동 백업을 끈 사용자도 자동 태그는 쓸 수 있어야 한다.

- 하루 한 번(`PeriodicWorkRequest`, `flex` 로 여유), 충전 중 + 배터리 여유일 때.
- 마지막 훑기 이후 추가된 사진만 대상이라 대개 몇 장이다.
- 설정에서 끄면 주기 워커를 취소한다.
...
- 상단: 켜기/끄기 스위치(새 사진 자동 분석), "지금 전체 분석" 버튼, 진행률, 마지막 분석 시각.
