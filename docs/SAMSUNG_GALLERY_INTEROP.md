...
2026-09-23. **기기 없이 쓴 문서다.** 확정한 것과 측정이 필요한 것을 나눠 적는다.
...
| **즐겨찾기** | **아마 된다 — 측정 필요** | 플랫폼 `IS_FAVORITE` 를 우리는 이미 쓴다. 삼성도 쓰는지가 관건 |
...
**삼성 갤러리가 같은 컬럼을 쓰는지는 확인하지 않았다.** 이 한 가지에 결론이 갈린다.

| 삼성이 `IS_FAVORITE` 를 쓴다면 | 안 쓴다면 |
|---|---|
| **이미 공유되고 있다.** 코드 변경 0 | 방법이 없다. 삼성 DB 에 쓸 길이 없다 |

추측하지 않는다. 아래 측정으로 5분 안에 판별된다.
...
혹시 외부에 열린 provider 가 있는지 보는 것인데, 있더라도 **문서화되지 않은 것이라
기대면 안 된다.** One UI 업데이트로 말없이 바뀐다.
...
1. **먼저 즐겨찾기 한 가지만 측정한다.** 이게 되면 얻는 게 가장 크고 비용이 0이다.
2. 앨범은 이미 되고 있으니 **문서에 그렇게 적어 두는 것**으로 충분하다. 사용자가 모를 뿐이다.
3. 그룹은 포기한다. 되는 척하지 않는다.
